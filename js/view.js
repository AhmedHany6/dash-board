...
const AUTH_TOKEN = "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..pRsjwKbOyvw6HZeqcuf6FL5yuvdgqPIIZiEt-Bvz_FY";
let usersData = [];
...
async function acceptUser(userId) {
    ...
    if (result.isConfirmed) {
        try {
            ...
            if (response.ok) {
                Swal.fire('Accepted!',
                   ` admin has accepted the user with ID: ${userId}`, 'success');
                await updateDashboard();
            } else {
...
async function viewDetails(userId) {
   ...
   const html = `
    ...
    <select id="statusSelect" class="swal2-select">
      <option value="reviewed" ${user.status === "reviewed" ? "selected" : ""}>Submitted</option>
<option value="accepted" ${user.status === "accepted" ? "selected" : ""}>reviewed</option>
<option value="rejected" ${user.status === "rejected" ? "selected" : ""}>accepted</option>
<option value="submitted" ${user.status === "submitted" ? "selected" : ""}>rejected</option>
...
function loadUserProfile() {
    ...
    if (img) {
        img.src = profileImage || "./images/pngtree-vector-users-icon-png-image_856952.jpg";
    }
}
